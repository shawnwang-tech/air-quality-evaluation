...
def time_series_smoothing(time_seires, window_size, method='mean'):
    ...
    print(len(time_seires.index))
    print(len(set(time_seires.index)))
    print(time_seires.index[0])
    print(time_seires.index[-1])
    #time_series_smooth = pd.DataFrame(index=times)
    time_series_smooth = pd.DataFrame(index=time_seires.index)
